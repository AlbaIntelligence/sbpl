...
public:
    ...
    py::safe_array<int> get_primitive_collision_pixels(int SourceTheta, int motprimID) const {

        std::vector<sbpl_2Dcell_t> collisionCells;
        _environment.GetCollisionCellsForPrimitive(SourceTheta, motprimID, &collisionCells);
        py::safe_array<int> result_array({(int)collisionCells.size(), 2});
        int* p_result = &result_array.mutable_unchecked()(0, 0);
        memcpy(p_result, &(collisionCells[0].x), sizeof(int)*collisionCells.size()*2);
...
class SBPLCollisionChecker {
public:

    SBPLCollisionChecker(
        const py::safe_array<double>& footprint_array,
        double res,
        const py::safe_array<double>& primitives_array
        )
    {
        auto footprint = footprint_array.unchecked<2>();
        std::vector<sbpl_2Dpt_t> polygon;
        polygon.resize(footprint_array.shape(0));
        memcpy(&(polygon[0].x), &footprint(0, 0), sizeof(sbpl_2Dpt_t)*polygon.size());

        auto primitves = primitives_array.unchecked<3>();
        for (int i=0; i<primitives_array.shape(0); ++i) {

            std::vector<sbpl_xy_theta_pt_t> poses;
            poses.resize(primitives_array.shape(1));
            memcpy(&(poses[0].x), &primitves(i, 0, 0), sizeof(sbpl_xy_theta_pt_t)*poses.size());

            std::vector<sbpl_2Dcell_t> new_cells;
            get_2d_motion_cells(
                polygon,
                poses,
                &new_cells,
            ...
            _primitive_cells.push_back(new_cells);
        }

    }

    void check_collisions(
        const py::safe_array<double>& costmap_array
    )
    {
...
private:
    std::vector<std::vector<sbpl_2Dcell_t>> _primitive_cells;
};


bool Costmap2D_worldToMap(
...
double CostmapModel_lineCost(
        const py::safe_array<unsigned int>& costmap_array,
        int x0, int x1, int y0, int y1
   )
{

    double line_cost = 0.0;
    auto costmap = costmap_array.unchecked<2>();

    int size_x = costmap_array.shape(1);
    int size_y = costmap_array.shape(0);
...
double CostmapModel_footprintCost(
    const py::safe_array<unsigned int>& costmap_array,
    double resolution,
    const sbpl_xy_theta_pt_t& position,
    const std::vector<sbpl_2Dpt_t>& footprint,
    ...
    double circumscribed_radius)
 {
    ...
    if(!Costmap2D_worldToMap(position.x, position.y, cell_x, cell_y, resolution))
      return -1.0;
    ...
    for(unsigned int i = 0; i < footprint.size() - 1; ++i){
      ...
      if(!Costmap2D_worldToMap(footprint[i].x, footprint[i].y, x0, y0, resolution))
        return -1.0;

      //get the cell coord of the second point
      if(!Costmap2D_worldToMap(footprint[i + 1].x, footprint[i + 1].y, x1, y1, resolution))
        return -1.0;

      line_cost = CostmapModel_lineCost(costmap_array, x0, x1, y0, y1);
    ...
    if(!Costmap2D_worldToMap(footprint.back().x, footprint.back().y, x0, y0, resolution))
      return -1.0;

    //get the cell coord of the first point
    if(!Costmap2D_worldToMap(footprint.front().x, footprint.front().y, x1, y1, resolution))
      return -1.0;
...
double footprintCost(
    ...
    double y,
    double theta,
    const std::vector<sbpl_2Dpt_t>& footprint_spec,
    double inscribed_radius,
    double circumscribed_radius)
{
    double cos_th = cos(theta);
    double sin_th = sin(theta);
    std::vector<sbpl_2Dpt_t> oriented_footprint;
    ...
    sbpl_xy_theta_pt_t robot_position;
    robot_position.x = x;
    robot_position.y = y;

    return CostmapModel_footprintCost(
        costmap_array, resolution, robot_position, oriented_footprint, inscribed_radius, circumscribed_radius);
}

void classic_dwa_obstacle_cost(
        const py::safe_array<unsigned int>& costmap_array,
        double resolution,
        const py::safe_array<double>& trajectory_array,
        const py::safe_array<double>& footprint_array,
        double inscribed_radius,
        double circumscribed_radius
    )
{
    ...
    trajectoryCosts.reserve(trajectory_array.shape(0));

    auto trajectories = trajectory_array.unchecked<3>();

    auto footprint_ref = footprint_array.unchecked<2>();
    std::vector<sbpl_2Dpt_t> footprint;
    footprint.resize(footprint_array.shape(0));
    memcpy(&(footprint[0].x), &footprint_ref(0, 0), sizeof(sbpl_2Dpt_t)*footprint.size());


    for (unsigned int traj_i = 0; traj_i < trajectory_array.shape(0); traj_i++) {
            // rough copypaste from https://github.com/ros-planning/navigation/blob/5a6ad24cf1bbc8ac08c06ef258fdf0bbdb3f9789/base_local_planner/src/obstacle_cost_function.cpp
        for (unsigned int i = 0; i < trajectory_array.shape(1); ++i) {
            const double* p_point = &(trajectories(traj_i, i, 0));

            double f_cost = footprintCost(
                costmap_array,
                resolution,
                *p_point, *(p_point+1), *(p_point+2),
                footprint,
                inscribed_radius,
    ...
    double circumscribed_radius)
            */
            trajectoryCosts.push_back(f_cost);
        }
    }
...
PYBIND11_MODULE(_sbpl_module, m) {
    ...
    py::class_<SBPLCollisionChecker>(m, "SBPLCollisionChecker")
        .def(py::init<const py::safe_array<double>&, double, const py::safe_array<double>&>())
        .def("check_collisions", &SBPLCollisionChecker::check_collisions)
    ;

    m.def("classic_dwa_obstacle_cost", &classic_dwa_obstacle_cost);
}
